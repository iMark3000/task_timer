...
def test_query_by_date_range():
    ...
    result = dbm.query_by_date_range(session_ids, start, end)
    for r in result:
...
def test_query_before_date():
    ...
    result = dbm.query_before_date(session_ids, query_date)
    for r in result:
        st = convert_date_string(r["start_timestamp"])
...
def test_query_after_date():
    ...
    result = dbm.query_after_date(session_ids, query_date)
    for r in result:
